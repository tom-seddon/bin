...
format_attrs=["nChannels","nSamplesPerSec","nAvgBytesPerSec","nBlockAlign","wBitsPerSample"]

Format=collections.namedtuple("Format",["wFormatTag"]+format_attrs)
...
def dump_fmt(fmt):
    ...
    for attr in format_attrs: sys.stdout.write("    %s=%s\n"%(attr,commas(getattr(g_format,attr))))
...
def store_format(chunk):
    global g_format

    if g_format is not None:
        sys.stderr.write("WARNING: file has more than one fmt chunk\n")

    g_format=Format(wFormatTag=get2u(chunk.data,0),
                    nChannels=get2u(chunk.data,2),
                    nSamplesPerSec=get4u(chunk.data,4),
                    nAvgBytesPerSec=get4u(chunk.data,8),
                    nBlockAlign=get2u(chunk.data,12),
                    wBitsPerSample=get2u(chunk.data,14))
...
def main(args):
    ...
    if args.duration:
        for chunk in chunks:
            if chunk.fourcc=="fmt ": store_format(chunk)
            elif chunk.fourcc=="data":
                total_seconds=len(chunk.data)/float(g_format.nAvgBytesPerSec)
        ...
        for chunk_idx in range(len(chunks)):
            ...
            if f is not None: f(chunk)
            elif len(chunk.data)<100: dump(chunk.data)
...
if __name__=="__main__":
    ...
    parser.add_argument("--duration",
                        action="store_true",
                        help="just print WAV file duration")
